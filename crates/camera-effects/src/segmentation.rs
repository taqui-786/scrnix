...
fn onnx_runtime_candidates(executable: Option<&Path>) -> Vec<PathBuf> {
    ...
    if let Some(exe_dir) = executable.and_then(Path::parent) {
        ...
        if let Some(contents_dir) = exe_dir.parent() {
            #[cfg(target_os = "linux")]
            candidates.push(contents_dir.join("lib/scrinx").join(ORT_LIBRARY_NAME));
...
mod tests {
    ...
    fn runtime_candidates_include_installed_and_relocated_linux_packages() {
        for (executable, library) in [
            ("/usr/bin/scrinx", "/usr/lib/scrinx/libonnxruntime.so"),
            (
                "/tmp/.mount_Scrinx/usr/bin/scrinx",
                "/tmp/.mount_Scrinx/usr/lib/scrinx/libonnxruntime.so",
            ),
        ] {
