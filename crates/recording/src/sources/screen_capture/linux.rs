...
impl PipewireCaptureState {
    fn set_fatal_error(&self, error: impl Into<String>) {
        if self.stop_requested.load(Ordering::Relaxed) || self.stop_token.is_cancelled() {
            return;
        }
...
fn capture_wayland(
    ...
    let _listener = stream
        ...
        .state_changed(|_, state, _, new| {
            if state.stop_requested.load(Ordering::Relaxed) || state.stop_token.is_cancelled() {
                return;
            }
    ...
    let error = if (stop_token.is_cancelled() || stop_requested.load(Ordering::Relaxed))
        && sent.load(Ordering::Relaxed) > 0
    {
        None
    } else {
        fatal_error.lock().take()
    };
    tracing::info!(
