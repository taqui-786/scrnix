...
pub fn appimage_alsa_config_path() -> Option<OsString> {
    ...
    let config = appdir.join("usr/lib/scrinx/alsa-pulse.conf");
...
pub fn updater_target(arch: &str) -> Result<String, String> {
    ...
    let marker = executable
        ...
        .and_then(|directory| fs::read_to_string(directory.join("lib/scrinx/package-format")).ok());
    let appimage = env::var_os("APPIMAGE").map(PathBuf::from);
    let appdir = env::var_os("APPDIR").map(PathBuf::from);
    let debian_files = fs::read_to_string("/var/lib/dpkg/info/scrinx.list").ok();
    target_for(
        package_format(
            &executable,
            marker.as_deref(),
...
mod tests {
    ...
    fn appimage_audio_preserves_host_and_custom_configuration() {
        let config = Path::new("/tmp/.mount_cap/usr/lib/scrinx/alsa-pulse.conf");
        for existing in [None, Some(OsStr::new(""))] {
            assert_eq!(
                append_alsa_config(config, existing).unwrap(),
                OsString::from(
                    "/usr/share/alsa/alsa.conf:/tmp/.mount_cap/usr/lib/scrinx/alsa-pulse.conf"
                )
            );
        }
        ...
        assert_eq!(
            combined,
            OsString::from(
                "/home/user/audio.conf:/home/user/devices.conf:/tmp/.mount_cap/usr/lib/scrinx/alsa-pulse.conf"
            )
        );
