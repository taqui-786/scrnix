...
fn is_local_recording(info: &VideoUploadInfo) -> bool {
    info.id.starts_with("local-")
}

fn local_video_upload_info(project_dir: &Path) -> VideoUploadInfo {
...
fn local_segment_uploader(recording_dir: &Path, video_id: &str) -> InstantUploader {
    let uploader = SegmentUploader {
        session: crate::upload::lifecycle::Session::new(
    ...
    #[cfg(target_os = "linux")]
    {
        Arc::new(tokio::sync::Mutex::new(uploader))
    }
                ...
                let actor_result: Result<InProgressRecording, anyhow::Error> = async {
                    ...
                    match inputs.mode {
                        ...
                        RecordingMode::Instant => {
                            ...
                            if let Some(events) = handle.take_segment_rx() {
                                tokio::task::spawn_blocking(move || while events.recv().is_ok() {});
                            }
                            let segment_upload =
                                local_segment_uploader(&recording_dir, &video_upload_info.id);
...
async fn discard_recording(app: &AppHandle, recording: InProgressRecording) -> Result<(), String> {
    ...
    if let Some(video_id) = video_id.filter(|video_id| !video_id.starts_with("local-")) {
...
async fn handle_recording_finish(
    ...
    let (meta_inner, sharing) = match completed_recording {
        ...
        CompletedRecording::Instant {
            ...
            if is_local_recording(&video_upload_info) {
                AppSounds::StopRecording.play();
                let _ = await_instant_upload(segment_upload).await;
                let inner = RecordingMetaInner::Instant(recording.meta.clone());
                if let Ok(mut meta) = RecordingMeta::load_for_project(&recording_dir) {
                    meta.inner = inner.clone();
                    meta.sharing = None;
                    meta.upload = None;
...
mod tests {
    ...
    #[test]
    fn recording_start_preflight_allows_instant_recordings_without_an_account() {
        assert_eq!(recording_start_mode_error(RecordingMode::Instant), None);
    }
...
pub(crate) mod linux_instant {
    ...
    async fn finalize(attempt: &Attempt, completed: &mut CompletedRecording) -> Result<(), String> {
        ...
        let mut meta =
            RecordingMeta::load_for_project(directory).map_err(|error| error.to_string())?;
        attempt.checked(Ok(()))?;
        if !super::is_local_recording(video_upload_info) {
